...
impl ContentExtractor {
    ...
    fn compute_aggregator_score(&self, document: &Html, text_content: &str) -> f32 {
        // Signal 1: RSS/Atom feed links (weight 0.15)
        let feed_score = self.detect_feed_links(document);

        // Signal 2: External link density (weight 0.50)
        let link_density_score = self.compute_external_link_density(document, text_content);

        // Signal 3: Content structure (weight 0.35)
        let structure_score = self.compute_structure_score(document, text_content);
    ...
    fn detect_feed_links(&self, document: &Html) -> f32 {
        let selectors = [
    ...
    fn compute_external_link_density(&self, document: &Html, text_content: &str) -> f32 {
    ...
    fn compute_structure_score(&self, document: &Html, text_content: &str) -> f32 {
        let word_count = text_content.split_whitespace().count();
